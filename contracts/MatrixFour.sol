...
contract MatrixFour is MatrixCore {
    ...
    function _search(uint256 id) public view returns (uint256) {
        uint256[] memory queue = new uint256[](364);
        queue[0] = id;
    ...
    function _makeRewards(uint256 _parentMatrixId) internal {
        uint256[5] memory uplineRewards =
            [
                msg.tokenvalue.mul(1).div(10),
                msg.tokenvalue.mul(3).div(20),
                msg.tokenvalue.mul(1).div(5),
                msg.tokenvalue.mul(1).div(4),
            ...
            if (matrix[uplineMatrixId].subtreeMatrixCount == 360) {
                if (
                    ...
                    (matrix[uplineMatrixId].userAddress ==
                        idToAddress[rootUserId])
                ) {
                    _nonBlockingTransfer(
                        idToAddress[rootUserId],
                        msg.tokenvalue.mul(2).div(10)
                    );
                } else {
                    _nonBlockingTransfer(
                        matrix[uplineMatrixId].userAddress,
